  ...
  getResult(symptoms: Symptoms, ppf: PoorPrognosticFactor) {
    ...
    if (symptoms.fever || (symptoms.cough && symptoms.soreThroat) || (symptoms.cough && symptoms.muscularPain) || (symptoms.fever && symptoms.diarrhea)) {
      ...
      if (fact_gravite_maj === 0 && fact_gravite_min === 0) {
        this.resultat = 'مافيكش فيروس كورونا على الأرجح، ولكن إلى شكيتي ممكن تتصل بطبيب ديالك';
      } else if (fact_gravite_min > 0 || fact_gravite_maj > 0 || fact_pronostique) {
        this.resultat = 'مافيكش فيروس كورونا على الأرجح ؛ تنصحوك تشاور مع طبيب. وإلى شكيتي اتصل ب 141';
      }
    } else {
      this.resultat = 'مافيكش فيروس كورونا على الأرجح، إذا عندك شك اتصل بالطبيب ديالك. تقد تعاود الإختبار لكانوا عندك اعراض اخرى. لبغيتي تعرف اكثر على الڤيروس ممكن تشوف صفحة النصائح';
    }
  }
